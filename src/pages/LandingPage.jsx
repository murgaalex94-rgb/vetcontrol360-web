...
function DemoModal({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4" onClick={function (e) { e.stopPropagation(); }}>
        <h2 className="text-2xl font-bold mb-6" style={{ color: DARK }}>Agenda una demo</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: GRAY }}>Nombre</label>
            <input type="text" className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none focus:ring-2 transition" style={{ borderColor: '#D1D5DB', color: DARK }} placeholder="Tu nombre" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: GRAY }}>Correo electrónico</label>
            <input type="email" className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none focus:ring-2 transition" style={{ borderColor: '#D1D5DB', color: DARK }} placeholder="correo@ejemplo.com" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: GRAY }}>Mensaje</label>
            <textarea rows={3} className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none focus:ring-2 transition resize-none" style={{ borderColor: '#D1D5DB', color: DARK }} placeholder="Cuéntanos sobre tu clínica..." />
          </div>
          <button className="w-full py-3 rounded-xl text-sm font-semibold text-white transition-all cursor-pointer hover:opacity-90" style={{ backgroundColor: PRIMARY }}>
            Enviar
          </button>
        </div>
        <button onClick={onClose} className="mt-4 text-sm w-full text-center cursor-pointer" style={{ color: GRAY }}>Cancelar</button>
      </div>
    </div>
  );
}
  ...
  return (
    <nav className="flex items-center justify-between px-8 py-4 max-w-7xl mx-auto w-full">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={onOpenDemo} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition-colors cursor-pointer" style={{ backgroundColor: PRIMARY }}>
          Agenda una demo
        </button>
      </div>
    </nav>
    ...
    <section id="inicio" className="flex-1 flex items-center px-8 max-w-7xl mx-auto w-full">
      <div className="grid md:grid-cols-2 gap-12 items-center w-full">
        <div className="space-y-6">
          ...
          <button onClick={onOpenDemo} className="inline-flex items-center gap-2 px-8 py-4 rounded-xl text-lg font-semibold text-white shadow-lg transition-all hover:opacity-90 cursor-pointer" style={{ backgroundColor: PRIMARY }}>
            Agenda una demo
...
function LandingPage() {
  var [demoOpen, setDemoOpen] = useState(false);

  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: BG }}>
      <DemoModal open={demoOpen} onClose={function () { setDemoOpen(false); }} />
      <Navbar onOpenDemo={function () { setDemoOpen(true); }} />
      <Hero onOpenDemo={function () { setDemoOpen(true); }} />
      <Features />
      <Pricing />
