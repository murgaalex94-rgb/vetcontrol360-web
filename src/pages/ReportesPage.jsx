...
function NuevoReporteModal({ onClose, onGenerar }) {
  var [form, setForm] = useState({ tipo: 'Ventas', formato: 'PDF', fechaDesde: '2026-07-01', fechaHasta: '2026-07-04' });
...
function ReportesPage() {
  ...
  var [fechaDesde, setFechaDesde] = useState('2026-07-01');
  var [fechaHasta, setFechaHasta] = useState('2026-07-04');
  ...
  function handleDescargar(reporte, formato) {
    console.log('Descargando ' + formato + ':', reporte);
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 rounded-xl border border-gray-300 dark:border-[#404040] bg-white dark:bg-[#2C2C2C] px-5 py-2.5 text-sm font-medium text-gray-700 dark:text-[#D0D0D0] hover:bg-gray-50 dark:hover:bg-[#333] transition-colors cursor-pointer">
              ...
              return (
                <div key={rep.id} onClick={function () { setReporteSeleccionado(rep); }} className={'p-4 rounded-xl border cursor-pointer transition-all ' + (activo ? 'border-[#5F7B65] bg-[#5F7B65]/5 shadow-sm' : 'border-gray-200 dark:border-[#333] hover:border-gray-300 dark:hover:border-[#404040] hover:bg-gray-50 dark:hover:bg-[#2C2C2C]')}>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <button onClick={function (e) { e.stopPropagation(); handleDescargar(rep, 'PDF'); }} className="flex items-center gap-1 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-3 py-1.5 text-xs font-medium text-red-700 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors cursor-pointer">
                    ...
                    <button onClick={function (e) { e.stopPropagation(); handleDescargar(rep, 'Excel'); }} className="flex items-center gap-1 rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 px-3 py-1.5 text-xs font-medium text-green-700 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors cursor-pointer">
              ...
              <div className="space-y-6">
                <div className="bg-gray-50 dark:bg-[#2C2C2C] rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-xl">{tipoIcono[reporteSeleccionado.tipo]}</span>
                    <h3 className="text-sm font-bold text-gray-900 dark:text-[#E0E0E0]">{reporteSeleccionado.titulo}</h3>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-2xl font-bold text-gray-900 dark:text-[#E0E0E0]">S/. 7,800</p>
                      <p className="text-xs text-gray-500 dark:text-[#909090]">Total</p>
                    </div>
                    <div>
                      <p className="text-2xl font-bold text-emerald-600">+12%</p>
                      <p className="text-xs text-gray-500 dark:text-[#909090]">vs. mes anterior</p>
                    </div>
                    <div>
                      <p className="text-2xl font-bold text-blue-600">48</p>
                      <p className="text-xs text-gray-500 dark:text-[#909090]">Registros</p>
                    </div>
                  </div>
                </div>

                <div>
                  <p className="text-sm font-semibold text-gray-700 dark:text-[#B0B0B0] mb-3">Ventas Mensuales</p>
                  <div className="bg-gray-50 dark:bg-[#2C2C2C] rounded-xl p-4" style={{ height: 200 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={ventasData}>
                        ...
                        <Tooltip />
                        <Bar dataKey="ventas" fill="#5F7B65" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-700 dark:text-[#B0B0B0] mb-3">Distribución por Categoría</p>
                  <div className="bg-gray-50 dark:bg-[#2C2C2C] rounded-xl p-4 flex items-center gap-6" style={{ height: 180 }}>
                    <div style={{ width: 140, height: 140 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={inventarioData} cx="50%" cy="50%" innerRadius={35} outerRadius={60} paddingAngle={3} dataKey="value">
                            {inventarioData.map(function (entry, index) {
                              return <Cell key={index} fill={coloresPie[index % coloresPie.length]} />;
                            })}
                          </Pie>
                          <Tooltip />
                        </PieChart>
                    ...
                    <div className="space-y-2 flex-1">
                      {inventarioData.map(function (item, i) {
                        return (
                          <div key={i} className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: coloresPie[i] }}></div>
                            <span className="text-xs text-gray-600 dark:text-[#A0A0A0] flex-1">{item.nombre}</span>
                            <span className="text-xs font-semibold text-gray-900 dark:text-[#E0E0E0]">{item.value}%</span>
                          </div>
                        );
                      })}
                    </div>
